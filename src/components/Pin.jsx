        ...
        <div className="m-2">
                    ...
                    postHovered && (
                        <div className="absolute top-0 w-full h-full flex flex-col justify-between p-1 pr-2 pt-2 pb-2 z-50" style={{ height: '100%' }}>
                            <div className="flex items-center justify-between">
                                ...
                                {
                                    alreadySaved  ? (
                                        <button onClick={(e) => e.stopPropagation()} type="button" className="bg-red-500 opacity-70 hover:opacity-100 text-white font-bold px-5 py-1 text-base rounded-3xl hover:shadow-md outline-none">
                                            { save?.length } Saved
                                        </button>
                                    ):
                                    (
            ...
            </div>
            {
                <Link to={`/user-profile/${postedBy._id}`} className="flex gap-2 mt-2 items-center">
                    ...
                    <p className="font-semibold captitalize">{postedBy?.userName}</p>
                </Link>
            }
